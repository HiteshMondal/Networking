...
function loadDemoData() {
    ...
    dashboardData = {
        ...
        history: all.slice(0, 8).map((s, i) => ({
            name: s,
            log_name: `${s}_20240118_12000${i}.log`,
...
function updateSysMeter(key, pct, level, subtitle) {
    const K = cap(key);
    const pctEl  = document.getElementById(`sys${K}Pct`);
...
function updateNetStats(net) {
    ...
    // FIX: use correct (non-duplicate) element IDs
    setText('sysNetErrors', (net.errin ?? 0) + (net.errout ?? 0));
    // Badge in the header of the network card
...
function updateRecentActivity() {
    ...
    history.slice(0, 6).forEach(item => {
        ...
        badge.className = `source-badge source-${sanitizeClass(item.category ?? 'other')}`;
        badge.textContent = item.category ?? 'other';

        // Source badge (script vs tool)
        const srcBadge = document.createElement('span');
        srcBadge.className = `log-type-tag type-${sanitizeClass(item.source ?? 'script')}`;
        srcBadge.textContent = item.source === 'tool' ? 'TOOL' : 'SCRIPT';
        ...
        hdr.appendChild(title); hdr.appendChild(badge); hdr.appendChild(srcBadge); hdr.appendChild(timeEl); hdr.appendChild(shareBtn);
...
//  Output Files — grouped by subdirectory 
...
function filterOutputFiles() {
    ...
    const outputs = dashboardData.outputs ?? [];
    const filtered = activeOutputGroup === 'all'
        ? outputs
        : outputs.filter(f => (f.subdir || '(root)') === activeOutputGroup);
...
function renderOutputFiles(outputs) {
    ...
    if (!outputs.length) {
        container.replaceChildren(buildEmptyState('◫', 'No output files found.'));
        return;
    }
    ...
    const groups = {};
    outputs.forEach(f => {
        const key = f.subdir || '(root)';
        (groups[key] = groups[key] || []).push(f);
    });

    // Container is a flat CSS grid — group headers span all columns,
    // file items sit directly in the grid and flow left-to-right naturally.
    const frag = document.createDocumentFragment();
    Object.entries(groups).sort(([a], [b]) => a.localeCompare(b)).forEach(([groupName, files]) => {
        // Group header spans full width via CSS class
        const header = document.createElement('div'); header.className = 'output-group-header';
        header.innerHTML = `<span class="output-group-icon">📁</span>
            <span class="output-group-name">${escHtml(groupName)}</span>
            <span class="output-group-count">${files.length} file${files.length !== 1 ? 's' : ''}</span>`;
        frag.appendChild(header);

        // File items go directly into the flat grid (no wrapping div)
        files.forEach(file => {
            const item = document.createElement('div');
            ...
            const viewBtn = document.createElement('button'); viewBtn.className = 'btn btn-ghost btn-sm file-view-btn';
            viewBtn.textContent = '👁 View'; viewBtn.title = 'View file';
            viewBtn.addEventListener('click', e => { e.stopPropagation(); openOutputFile(file); });
            const shareBtn = document.createElement('button'); shareBtn.className = 'btn btn-ghost btn-sm file-share-btn';
            shareBtn.textContent = '↗'; shareBtn.title = 'Share';
...
// Open a text output file in the log viewer modal
function openOutputFile(fileObj) {
    stopTail();
    currentLogFile = fileObj;
    showModal(fileObj.name, fileObj.dir ?? 'outputs', fileObj.name_param ?? fileObj.name);
}
...
function renderHistoryRows(items) {
    ...
    if (!items?.length) {
        ...
        td.setAttribute('colspan', '8'); td.className = 'empty-row'; td.textContent = 'No matching results';
...
async function openTail(logObj) {
    ...
    content.textContent = 'Connecting to live tail…';
    ...
    try {
        ...
        data.results.forEach(r => {
            ...
            row.addEventListener('click', () => {
                ...
                openLog(log ?? { name: r.file, dir: r.dir ?? 'logs', name_param: r.file });
                clearSearchResults();
            });
...
async function downloadFile(fileObj) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            showToast('Download failed — server returned error', 'error');
        }
