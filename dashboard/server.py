#!/usr/bin/env python3
"""CyberDeck Dashboard Server v3.1 — fixed log/output scanning, system stats, email alerts."""
...
#  Comprehensive category + source mapping 
_CATEGORY_MAP = {
    # scripts
    'detect_suspicious_net_linux':   'network',
    'detect_suspicious_net_linux.sh':'network',
    'secure_system':      'security',
    'secure_system.sh':   'security',
    'revert_security':    'security',
    'revert_security.sh': 'security',
    'system_info':        'system',
    'system_info.sh':     'system',
    'forensic_collect':   'forensic',
    'forensic_collect.sh':'forensic',
    'web_recon':          'recon',
    'web_recon.sh':       'recon',
    'malware_analysis':   'security',
    'malware_analysis.sh':'security',
    'lateral_movement_detect':    'security',
    'lateral_movement_detect.sh': 'security',
    'log_analysis':       'system',
    'log_analysis.sh':    'system',
    'cloud_exposure_audit':    'network',
    'cloud_exposure_audit.sh': 'network',
    'data_exfil_detect':       'security',
    'data_exfil_detect.sh':    'security',
    # tools
...
# Names that belong to the "tools" source (not scripts)
_TOOL_NAMES = {
...
def _stem_from_name(filename: str) -> str:
    """Extract the base script/tool stem from a log filename.

    Handles both:
      script_name_20240118_120000.log   → script_name
      tool_name_20260307_154820.log     → tool_name
    Also strips a trailing .sh if present.
    """
    name = Path(filename).stem          # strip .log
    # Match timestamp suffix _YYYYMMDD_HHMMSS
    m = re.match(r'^(.+?)_(\d{8}_\d{6})$', name)
    stem = m.group(1) if m else name
    # Strip trailing .sh (some logs are named "script.sh_timestamp")
    if stem.endswith('.sh'):
        stem = stem[:-3]
    return stem
...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        p, qs = urlparse(self.path), parse_qs(urlparse(self.path).query)
        {
            ...
            '/api/alert-settings': lambda: self._json_response(200, {
                **_alert_cfg,
    ...
    def _serve_file(self, qs):
        rd  = qs.get('dir',  [''])[0]
        rn  = qs.get('name', [''])[0]
        # Support subdirectory path within outputs, e.g. "exfil_detect/dns_tunnelling.txt"
        if rd == 'logs':
            bd = LOGS_DIR
        elif rd == 'outputs':
            bd = OUTPUT_DIR
        else:
            return self._send_error(400, "dir must be 'logs' or 'outputs'")
        if not rn:
    ...
    def _serve_search(self, qs):
        ...
        if len(q) < 2:
            return self._json_response(200, {'results': [], 'query': q, 'total': 0})
            ...
            for f in sorted(LOGS_DIR.rglob('*.log'),
                ...
                try:
                    for no, line in enumerate(
                        f.read_bytes()[:512_000].decode('utf-8', errors='replace').splitlines(), 1
                    ):
                        if pat.search(line):
                            # Return path relative to LOGS_DIR so client can fetch it
    ...
    def _serve_metrics(self):
        ...
        for x in h:
            try:
                by_day[x['timestamp'][:10]][x.get('status', 'unknown')] += 1
                by_day[x['timestamp'][:10]]['total'] += 1
            except Exception:
                pass
    ...
    def _serve_categories(self):
        self._json_response(200, {
            'categories': sorted({h.get('category', 'other') for h in self._get_history()})
        })
    ...
    def _handle_notify_email(self, payload):
        ...
        if not to:
            return self._send_error(400, "Missing 'to'")
        if not (SMTP_USER and SMTP_PASS):
            return self._send_error(503, 'SMTP not configured. Set SMTP_USER and SMTP_PASS env vars.')
        ...
        if ok:
            self._json_response(200, {'sent': True, 'to': to})
        else:
            self._send_error(500, f'Email failed: {msg}')
    ...
    def _smtp_send(self, to, subject, body):
        try:
            ...
            msg.attach(MIMEText(
                ...
                f'<p style="color:#546278;font-size:12px">Networking & Cybersecurity Automation Toolkit'
                f'</p></body></html>', 'html',
            ))
    ...
    #  File helpers 
    def _get_logs(self):
        """Return all .log files under LOGS_DIR (recursive), excluding dashboard.log."""
        if not LOGS_DIR.is_dir():
            return []
        logs = []
        for f in LOGS_DIR.rglob('*.log'):
            if not f.is_file() or f.name == 'dashboard.log':
                continue
            stem = _stem_from_name(f.name)
            st   = f.stat()
            # Relative path from LOGS_DIR for the name_param (used in /api/file)
            rel  = str(f.relative_to(LOGS_DIR))
            logs.append({
                'name':       f.name,
                'script':     stem,
                ...
                'size':       st.st_size,
                'dir':        'logs',
                'name_param': rel,        # may be "subdir/file.log" — _serve_file handles it
                'source':     'tool' if stem in _TOOL_NAMES else 'script',
                'category':   self._cat(stem),
            })
        return sorted(logs, key=lambda x: x['timestamp'], reverse=True)

    def _get_outputs(self):
        """Return all files under OUTPUT_DIR recursively, including subdirectories."""
        ...
        for f in OUTPUT_DIR.rglob('*'):
            if not f.is_file():
                continue
            st  = f.stat()
            rel = str(f.relative_to(OUTPUT_DIR))   # e.g. "exfil_detect/dns_tunnelling.txt"
            # Display name: show subdir context if nested
            display_name = rel if '/' in rel else f.name
            out.append({
                'name':       display_name,
    ...
    def _get_history(self):
        """Build execution history from log files."""
        if not LOGS_DIR.is_dir():
            return []
        hist = []
        for f in LOGS_DIR.rglob('*.log'):
            if not f.is_file() or f.name == 'dashboard.log':
                continue
            stem = _stem_from_name(f.name)
            # Parse timestamp from filename, fall back to mtime
    ...
    def _parse_log(path: Path):
        ...
        if m:
            st = 'success' if m.group(1) == '0' else 'error'
...
def main():
    ...
    print('=' * 58, f'\n  CyberDeck Dashboard Server v3.1\n' + '=' * 58)
    if not HAS_PSUTIL:
